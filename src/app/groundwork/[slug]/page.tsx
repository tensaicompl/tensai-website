...
export async function generateStaticParams() {
  return getConceptsByPillar("groundwork").map((c) => ({ slug: c.slug }));
}
...
export async function generateMetadata({
  ...
  const concept = getConceptBySlug("groundwork", slug);
  if (!concept) return {};
  return {
    title: `${concept.frontmatter.title} — The Groundwork — TensAI`,
    description: concept.frontmatter.definition,
  };
}

export default async function GroundworkConceptPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const allConcepts = getConceptsByPillar("groundwork");
        ...
        >
          ...
          <nav aria-label="Breadcrumb">
            <div className="breadcrumb">
              ...
              <Link href="/groundwork" className="breadcrumb-link">
                The Groundwork
              </Link>
              <span className="breadcrumb-sep" aria-hidden="true">
                /
          ...
          {(prevConcept || nextConcept) && (
            ...
            >
              <div>
                {prevConcept && (
                  <Link
                    href={`/groundwork/${prevConcept.slug}`}
              ...
              <div style={{ textAlign: "right" }}>
                {nextConcept && (
                  <Link
                    href={`/groundwork/${nextConcept.slug}`}
                    className="concept-nav-link"
                  >
