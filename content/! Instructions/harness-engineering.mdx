...
The harness has a precise vocabulary, and it is worth learning exactly. Its core distinction — guides and sensors — is borrowed from control-systems theory, and is owed in its current form to Martin Fowler. Every control in a harness is one of these two kinds, and the difference between them is simply *when* they act.
...
TensAI maintains a working reference harness — a starting `AGENTS.md`, a layered stack of computational sensors (type-check, lint, test), and an evaluator-model sensor wired into the iterate-the-harness loop — at [`tensaicompl/harness-template`](https://github.com/tensaicompl/harness-template). Clone it to see guides and sensors operating together before building your own.
</Callout>
